...
struct ObservabilityClassifyResponseTests {
  ...
  func `nonRetryable reason omits body excerpt when not requested`() {
    ...
    _ = NetworkingUtils.classifyResponse(
      statusCode: 429,
      retryAfterHeader: nil,
      partialSuccess: nil,
      bodyExcerpt: {
...
@Suite("NetworkingUtils.parseRetryAfter")
struct ObservabilityParseRetryAfterTests {
  @Test
  func `nil header returns nil`() {
    #expect(NetworkingUtils.parseRetryAfter(nil) == nil)
  }

  @Test
  func `empty or whitespace header returns nil`() {
    #expect(NetworkingUtils.parseRetryAfter("") == nil)
    #expect(NetworkingUtils.parseRetryAfter("   ") == nil)
  }

  @Test
  func `integer seconds parses to TimeInterval`() {
    #expect(NetworkingUtils.parseRetryAfter("0") == 0)
    #expect(NetworkingUtils.parseRetryAfter("30") == 30)
    #expect(NetworkingUtils.parseRetryAfter("3600") == 3600)
  }
  ...
  func `negative seconds clamps to zero`() {
    ...
    #expect(NetworkingUtils.parseRetryAfter("-1") == 0)
  }
  ...
  func `fractional seconds parse`() {
    ...
    #expect(NetworkingUtils.parseRetryAfter("0.5") == 0.5)
  }
  ...
  func `HTTP-date header parses to delta from now`() {
    ...
    let parsed = NetworkingUtils.parseRetryAfter(header)
    #expect(parsed != nil)
    if let parsed {
  ...
  func `HTTP-date in the past clamps to zero`() {
    ...
    #expect(NetworkingUtils.parseRetryAfter(formatter.string(from: past)) == 0)
  }

  @Test
  func `garbage header returns nil`() {
    #expect(NetworkingUtils.parseRetryAfter("tomorrow morning") == nil)
    #expect(NetworkingUtils.parseRetryAfter("Mon Jun 16") == nil)
  }
}
